...
-- Idea is to have a radius pickup range around the player, whatever the height is
...
-- entities dropped by mining are instantly picked up, everything else (player dropped) are not
local delay_before_playerdrop_pickup = 1
-- The time the entities has to come to the player (this value is modifiable)
						...
						elseif not luaEnt.item_drop_nopickup then
							...
							if playerDistance <= pickup_range_squared then
								...
								if inv:room_for_item("main", itemStack) then
									...
									-- Marks the object as already being picked up
									...
									minetest.after(pickup_duration, function()
										...
										-- sounds can be changed in the minetest.sound_play string, as well as max_hear_distance
										if inv:room_for_item("main", itemStack) then
